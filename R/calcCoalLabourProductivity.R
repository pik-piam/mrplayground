#' CalcCoalEmployment
#' calculates historical employment in the coal sector and projects current (2020) and future employment factors
#' @author Aman Malik
...
if (subtype=="Employment_factor"){
  ...
  ef_fut[,2020,] <-   c(4.4,42,rep(50,9),15,28.5,16,3.6,15.6)
  imp <- new.magpie(getRegions(ef_fut),seq(2025,2050,5))
  imp["AUS",c(2025,2030),] <- 1.5
  imp["AUS",seq(2030,2050,5),] <- -0.2
  imp["CHN",c(2025,2030),] <- -8
  imp["CHN",seq(2030,2050,5),] <- -3
  imp[regs,c(2025,2030),] <- -2.5
  imp[regs,seq(2030,2050,5),] <- -1.5
  imp["IDN",c(2025,2030),] <- -7
  imp["IDN",seq(2030,2050,5),] <- -3
  imp["IND",c(2025,2030),] <- -7
  imp["IND",seq(2030,2050,5),] <- -3
  imp["RUS",c(2025,2030),] <- -6
  imp["RUS",seq(2030,2050,5),] <- -3
  imp["USA",c(2025,2030),] <- -1.3
  imp["USA",seq(2030,2050,5),] <- -0.5
  imp["ZAF",c(2025,2030),] <- 2
  imp["ZAF",seq(2030,2050,5),] <- -1.5
  
  for (i in seq(2025,2050,5)){
  ...
  ef_fut <- toolCountryFill(ef_fut,fill = 0)
